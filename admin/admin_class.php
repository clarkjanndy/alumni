...
Class Action {
	...
	function save_register(){
		extract($_POST);
		$data = "id='$id', event='$event', event_id='$event_id',payment_type='$payment_type',type='$type',";
		$data .= "amount='$amount', schedule='$schedule', name='$name', email='$email', contact='$contact', address='$address', payment_status='$payment_status', status='$status'";
		$save = $this ->db->query("INSERT INTO audience $data");
	}
	...
	function delete_alumni(){
		extract($_POST);
		$commit = $this->db->query("DELETE FROM alumnus_bio WHERE id='".$id."'");
		if($commit)
			return 1;
	}
}